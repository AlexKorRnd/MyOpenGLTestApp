...
import static com.example.myapplication2.tg.AndroidUtilities.dp;
...
public class SpoilerEffect extends Drawable {
    ...
    private Stack<TgParticle> particlesPool = new Stack<>();
    ...
    private ArrayList<TgParticle> particles = new ArrayList<>();
    ...
    public void updateMaxParticles() {
        setMaxParticlesCount(MathUtils.clamp((getBounds().width() / dp(6)) * PARTICLES_PER_CHARACTER, PARTICLES_PER_CHARACTER, MAX_PARTICLES_PER_ENTITY));
    }
    ...
    public void startRipple(float rX, float rY, float radMax, boolean reverse) {
        ...
        rippleAnimator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                Iterator<TgParticle> it = particles.iterator();
                while (it.hasNext()) {
                    TgParticle p = it.next();
    ...
    public void draw(@NonNull Canvas canvas) {
        Log.i("test__________6", "draw:: drawPoints = " + drawPoints);
        if (drawPoints) {
            ...
            for (int i = 0; i < particles.size(); i++) {
                TgParticle particle = particles.get(i);
            ...
            if (particles.size() < maxParticles) {
                ...
                for (int i = 0; i < np; i++) {
                    ...
                    TgParticle newParticle = !particlesPool.isEmpty() ? particlesPool.pop() : new TgParticle();
                    int attempts = 0;
                    do {
...
//            }
    ...
    private boolean isOutOfBounds(int left, int top, int right, int bottom, float x, float y) {
        if (x < left || x > right || y < top + dp(VERTICAL_PADDING_DP) ||
                y > bottom - dp(VERTICAL_PADDING_DP))
            return true;
    ...
    private void generateRandomLocation(TgParticle newParticle, int i) {
    ...
    public void setMaxParticlesCount(int maxParticles) {
        ...
        while (particlesPool.size() + particles.size() < maxParticles) {
            particlesPool.push(new TgParticle());
        }
    }
    ...
    public static void addSpoilers(TextView tv, @Nullable Stack<SpoilerEffect> spoilersPool, List<SpoilerEffect> spoilers, ArrayList<Object> quoteBlocks) {
        int width = tv.getMeasuredWidth();
     ...
     * @param spoilers     Spoilers list to populate
     */
    public static void addSpoilers(@Nullable View v, Layout textLayout, int layoutLeft, int layoutRight, Spanned spannable, @Nullable Stack<SpoilerEffect> spoilersPool, List<SpoilerEffect> spoilers, ArrayList<Object> quoteBlocks) {
        if (textLayout == null) {
            return;
        }
    ...
    private static void addSpoilerRangesInternal(@Nullable View v, @NonNull Layout textLayout, int mostleft, int mostright, int start, int end, @Nullable Stack<SpoilerEffect> spoilersPool, List<SpoilerEffect> spoilers, ArrayList<Object> quoteBlocks) {
    ...
    private static void addSpoilerRangeInternal(@Nullable View v, @NonNull Layout textLayout, float left, float top, float right, float bottom, @Nullable Stack<SpoilerEffect> spoilersPool, List<SpoilerEffect> spoilers, int mostleft, int mostright, ArrayList<Object> quote) {
        ...
        if (!spoilers.isEmpty()) {
            ...
            for (SpoilerEffect eff : spoilers) {
                ...
                if (eff.shouldInvalidateColor()) {
                    eff.setColor(ColorUtils.blendARGB(spoilersColor, patchedLayoutType == 1 ? textLayout.getPaint().getColor() : Color.GREEN, Math.max(0, eff.getRippleProgress())));
                } else {
