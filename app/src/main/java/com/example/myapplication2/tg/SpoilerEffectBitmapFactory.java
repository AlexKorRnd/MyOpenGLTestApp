...
public class SpoilerEffectBitmapFactory {
    ...
    private SpoilerEffectBitmapFactory() {
        int maxSize = AndroidUtilities.dp(100);
    ...
    public void setupSpoiler(SpoilerEffect spoilerEffect) {
        int step = (int) (size / 10f);
        int particleCount = (int) (60 * (size / (float) AndroidUtilities.dp(200)));
        spoilerEffect.setSize(size);
        //spoilerEffect.setBounds(step * i, step * j - AndroidUtilities.dp(5), step * i + step +  AndroidUtilities.dp(3), step * j + step + AndroidUtilities.dp(5));
        spoilerEffect.drawPoints = true;
        spoilerEffect.particlePoints = new float[SpoilerEffect.ALPHAS.length][particleCount * 2];
        spoilerEffect.setMaxParticlesCount(particleCount);
        spoilerEffect.setColor(Color.WHITE);
    }
